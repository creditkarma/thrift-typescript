...
function readGenerated(name: string, location: string = 'generated'): string {
    return fs.readFileSync(path.join(__dirname, `./${location}/${name}/index.ts`), 'utf-8')
}
...
describe('Thrift TypeScript Generator', () => {
    describe('Thrift Server Generated', () => {
        before(() => {
            generate({
                rootDir: __dirname,
                outDir: 'generated-interfaces',
                sourceDir: 'fixtures/thrift',
                target: 'thrift-server-interfaces',
                files: [],
                i64Type: 'number',
            })
        })
        ...
        it('should correctly generate typedefs for includes', () => {
            const actual: string = readGenerated('operation', 'generated-interfaces')
            const expected: string = readGeneratedSolution('operation', 'generated-interfaces')
            assert.deepEqual(actual, expected)
        })

        it('should correctly generate a struct using includes', () => {
            const actual: string = readGenerated('common', 'generated-interfaces')
            const expected: string = readGeneratedSolution('common', 'generated-interfaces')
            assert.deepEqual(actual, expected)
        })

        it('should correctly generate an exception using includes', () => {
            const actual: string = readGenerated('exceptions', 'generated-interfaces')
            const expected: string = readGeneratedSolution('exceptions', 'generated-interfaces')
            assert.deepEqual(actual, expected)
        })

        it('should correctly generate a service', () => {
            const actual: string = readGenerated('shared', 'generated-interfaces')
            const expected: string = readGeneratedSolution('shared', 'generated-interfaces')
            assert.deepEqual(actual, expected)
        })
    ...
    describe('Interfaces', () => {
        it('should correctly generate a const', () => {
            ...
            `
            const actual: string = make(content, 'thrift-server-interfaces')
            const expected: string = readSolution('basic_const', 'thrift-server-interfaces')

            assert.deepEqual(actual, expected)
        })

        it('should correctly generate a const where i64Type is "string"', () => {
            const content: string = `
                const bool FALSE_CONST = false
                const i64 INT_64 = 64
                const set<string> SET_CONST = ['hello', 'world', 'foo', 'bar']
                ...
                const list<string> LIST_CONST = ['hello', 'world', 'foo', 'bar']
            `
            const actual: string = make(content, 'thrift-server-interfaces', 'string')
            const expected: string = readSolution('basic_const_string', 'thrift-server-interfaces')
        ...
        it('should correctly generate a struct', () => {
            ...
            `
            const expected: string = readSolution('multi_field_struct', 'thrift-server-interfaces')
            const actual: string = make(content, 'thrift-server-interfaces')
        ...
        it('should correctly generate a basic service', () => {
            ...
            `
            const expected: string = readSolution('basic_service', 'thrift-server-interfaces')
            const actual: string = make(content, 'thrift-server-interfaces')
